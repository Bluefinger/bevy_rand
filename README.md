...
```toml
rand_core = "0.10"
bevy_rand = { version = "0.16", features = ["chacha20", "wyrand"] }
```
...
rand_core = "0.10"
bevy_rand = "0.16"
bevy_prng = { version = "0.16", features = ["chacha20", "wyrand"] }
```
...
```toml
bevy_rand = { version = "0.16", default-features = false, features = ["chacha20", "wyrand"] }
```
...
From `v0.9` onwards, `bevy_rand` no longer assumes that `bevy` will be run in a web environment when compiled for WASM. To enable that in `v0.16`, just paste the following into your `Cargo.toml` for your binary crate:

```toml
[target.'cfg(all(target_family = "wasm", any(target_os = "unknown", target_os = "none")))'.dependencies]
bevy_rand = { version = "0.16", features = ["wasm_js"] }
```
