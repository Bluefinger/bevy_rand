    ...
    struct $newtype:ident($rng:ty);
        ...
        impl ::rand_core::SeedableRng for $newtype {
            ...
            fn seed_from_u64(seed: u64) -> Self {
                Self::new(<$rng>::seed_from_u64(seed))
            }
            ...
            fn default() -> Self {
                ...
                #[cfg(feature = "thread_local_entropy")]
                {
                    let mut local = super::thread_local_entropy::ThreadLocalEntropy::get()
                        .expect(crate::NO_ENTROPY_SOURCE);

                    Self::from_rng(&mut local)
                }
                #[cfg(not(feature = "thread_local_entropy"))]
                {
                    let mut source = getrandom::SysRng;

                    Self::try_from_rng(&mut source).expect(crate::NO_ENTROPY_SOURCE)
                }
            }
        }
...
#[cfg(any(feature = "chacha20", feature = "quality_rng"))]
macro_rules! chacha_prng {
    { #[feature = $feature:literal]
    $($(#[$doc:meta])*
            ...
            fn default() -> Self {
                ...
                #[cfg(not(feature = "thread_local_entropy"))]
                {
                    let mut seed: <$internal as ::rand_core::SeedableRng>::Seed =
                        Default::default();
                    ::getrandom::fill(seed.as_mut()).expect(crate::NO_ENTROPY_SOURCE);
                    Self::from_seed(seed)
                }
            }
        }
